...
public class DateUtil extends GregorianCalendar{
	...
	public static String formatDaT(Date paramDate) {
		if (paramDate == null)
			return null;
	...
	public static String[] getDateArrayBetween(String start,String end,int period){
		String startTime;
		String endTime;
		List<String> dateArray = new ArrayList<>();
        String ss = start.split(":")[1];
		String es = end.split(":")[1];
		int s = Integer.parseInt(ss);
		int e = Integer.parseInt(es);
		if(s%2!=0){
			startTime = addDateMinut(start,1);
		}else{
			startTime = start;
		}
		if(e%2!=0){
			endTime = addDateMinut(end,-1);
		}else{
			endTime = end;
		}
		dateArray.add(startTime);
		while (!startTime.equals(endTime)) {
			startTime = addDateMinut(startTime, period);
			dateArray.add(startTime);
		}
		//dateArray.add(endTime);
		int size=dateArray.size();
		String[] array = dateArray.toArray(new String[size]);
		return 	array;
	}
...
/*	public static void main(String[] args){
		String[] date = DateUtil.getDateArrayBetween("2016-05-16 11:09","2016-05-17 15:29");
